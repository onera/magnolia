...
% Add OSQP to path
addpath("C:\Users\thoma\Desktop\ONERA\V6\osqp-0.6.2-matlab-windows64");
...
try rmdir('osqp_c_code', 's'); catch; end
fprintf('[done]\n');
...
% Code generation
dir = 'osqp_c_code';
solver.codegen(dir, 'parameters', 'vectors');

% Code cleaning
clean_osqp_code(dir);

% Parameters manual definitions
fid = fopen('config_mpc.h', 'wt');
...
% MEX file build
inc_dir = fullfile('osqp_c_code', 'include');
src_dir = fullfile('osqp_c_code', 'src', 'osqp');
mex(['-I' inc_dir], 'osqp_build.c', fullfile(src_dir, '*.c'));

% Workspace cleaning
clear q l u fid inc_dir dir p_hat p_ref src_dir solver ans;
rmpath('C:\Users\thoma\Desktop\ONERA\V6\osqp-0.6.2-matlab-windows64');
