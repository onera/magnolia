...
void mpc_wrapper_step(double* X_est, double* X_ref_horizon, double* U_optimal_out, 
    ...
    state->epsilon[0] += Ts_mpc * (X_ref_horizon[0 * stride] - X_est[0]); 
    state->epsilon[1] += Ts_mpc * (X_ref_horizon[1 * stride] - X_est[1]); 
    state->epsilon[2] += Ts_mpc * (X_ref_horizon[2 * stride] - X_est[2]); 

    c_float X_aug[9];
    X_aug[0] = (c_float)X_est[0]; 
    X_aug[1] = (c_float)X_est[1];
    X_aug[2] = (c_float)X_est[2]; 
    X_aug[3] = (c_float)X_est[3]; 
    X_aug[4] = (c_float)X_est[4];
    X_aug[5] = (c_float)X_est[5]; 
    ...
    for (int j = 0; j < Np; j++) {
        double rx = X_ref_horizon[0 * stride + j];
        double ry = X_ref_horizon[1 * stride + j];
        double rz = X_ref_horizon[2 * stride + j];
    ...
    X_ref_terminal[0] = X_ref_horizon[0 * stride + Np]; 
    X_ref_terminal[1] = X_ref_horizon[1 * stride + Np]; 
    X_ref_terminal[2] = X_ref_horizon[2 * stride + Np]; 

    for (int i = 0; i < n; i++) {
        double double_sum = 0.0;
        for (int k = 0; k < n; k++) {
            double_sum += param->Q_Terminal[i][k] * X_ref_terminal[k];
        }
        state->q_new[idx_q++] = (c_float)(-double_sum);
    }
