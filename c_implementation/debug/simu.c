...
void get_mpc_prediction_horizon(unsigned long loop_counter, double shift_seconds, const char* traj_type, const Param* p, double* X_ref, double max_sim_time) {
    int stride = PARAM_NP + 1;
    double dt_mahony = 1.0 / PARAM_F_MAHONY;

    int dt_ratio = (int)round(PARAM_F_MAHONY / PARAM_F_MPC); 
    int shift_idx = (int)round(shift_seconds * PARAM_F_MAHONY);

    int current_k_minus_1 = (int)loop_counter + 1;

    int N_ext = (int)round((max_sim_time + ((double)PARAM_NP / PARAM_F_MPC) + shift_seconds) * PARAM_F_MAHONY) + 1; 

    for (int step = 0; step <= PARAM_NP; step++) {
...
int run_simulation(const char* traj_type, const char* log_filename, double max_sim_time, double shift_seconds, const char* log_format) {
    Param param;
    load_param(&param);
    ...
    while (sim_time <= max_sim_time + 1.0e-3) {
        ...
        if (loop_counter % (unsigned int)(param.f_plant / param.f_MoCap) == 0)  mocap_outputs(mocap_meas, &param, &sensors_state);
        ...
        double dt_m = 1.0 / PARAM_F_MAHONY;
        ...
        if (loop_counter % (unsigned int)(param.f_plant / param.f_kalman) == 0)  kalman_update(buffer_u, Y_meas, acc_meas, &kalman_state, &param);
        if (loop_counter % (unsigned int)(param.f_plant / param.f_mpc) == 0) {
            for (int i = 0; i < 3; i++) {
                X_hat_mpc[i] = kalman_X_hat[i];
                X_hat_mpc[i+3] = kalman_X_hat[i+6];
        ...
        if (loop_counter % (unsigned int)(param.f_plant / param.f_MoCap) == 0)  mocap_update(X, &param, &sensors_state);
...
int main(void) {
    const char* trajectories[] = {"Castle"};
    double run_times[]        = {230.0};
    int num_scenarios = sizeof(trajectories) / sizeof(trajectories[0]);
    
    double SHIFT_SECONDS = 2.5;
    const char* FORMAT_FLAG = "HEX"; 

    printf("====================================================\n");
    printf("     STARTING EMBEDDED C BATCH SIMULATION CAMPAIGN \n");
    printf("====================================================\n");
    ...
    for (int i = 0; i < num_scenarios; i++) {
        ...
        run_simulation(trajectories[i], out_filename, run_times[i], SHIFT_SECONDS, FORMAT_FLAG);
    }
